document.addEventListener("DOMContentLoaded", () => {
  ...
  const inputLoginApiKey = document.getElementById("input-login-api-key");
  ...
  chrome.storage.local.get(["backendUrl", "apiKey"]).then(({ backendUrl, apiKey }) => {
    ...
    if (apiKey) {
      ...
      inputLoginApiKey.value = apiKey;
    }
  });

  btnEnterDashboard.addEventListener("click", async () => {
    const backendUrl = inputLoginBackendUrl.value.trim().replace(/\/$/, "");
    const apiKey = inputLoginApiKey.value.trim();
    
    if (!backendUrl || !apiKey) {
      authError.textContent = "Please enter both Backend URL and API Key";
      authError.style.display = "block";
      setTimeout(() => { authError.style.display = "none"; }, 5000);
      return;
    }
    
    btnEnterDashboard.disabled = true;
    btnEnterDashboard.innerHTML = 'Connecting...';
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Invalid API Key or server not reachable");
      }
      
      // Save credentials
      await chrome.storage.local.set({ apiKey: apiKey, apiUrl: backendUrl, backendUrl: backendUrl });
      
      // Update the connection tab inputs as well
      const inputApiUrl = document.getElementById("input-api-url");
      const inputApiKeyConfig = document.getElementById("input-api-key");
      if (inputApiUrl) inputApiUrl.value = backendUrl;
      if (inputApiKeyConfig) inputApiKeyConfig.value = apiKey;
      ...
      btnEnterDashboard.innerHTML = 'Connect';
    }
  });
