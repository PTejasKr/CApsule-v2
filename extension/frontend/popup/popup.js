document.addEventListener("DOMContentLoaded", async () => {
  ...
  const changedWorkflowsCountEl = document.getElementById("changed-workflows-count");
  ...
  function renderCurrentPR(data) {
    ...
    if (wf.severity === "major") {
      ...
      riskScoreEl.textContent = "Medium";
      riskScoreEl.style.color = "var(--accent-yellow, #facc15)";
    } else {
  ...
  function formatChangelogText(changelog) {
    ...
    changedWorkflowsCountEl.textContent = wfCount;
  ...
  async function checkAuth() {
      const { apiKey, apiUrl, backendUrl } = await new Promise((resolve) => chrome.storage.local.get(["apiKey", "apiUrl", "backendUrl"], resolve));
      const backend = apiUrl || backendUrl;
