...
from extension.backend.worker import celery_app
...
async def _core_pr_analysis(repo: str, pr_number: int) -> dict:
    ...
    from extension.backend.services.integrations import ChatOpsService, IssueTrackerService
    from extension.backend.services.plugin_manager import plugin_manager
    from extension.backend.services.brd_manager import BRDManager
    from extension.backend.database import insert, fetch_one
    from extension.backend.middleware.security import sanitize_text
    from extension.backend.services.crypto import decrypt_token
    ...
    if row:
        profile = {
            ...
            "github_token": decrypt_token(row["github_token"]) if row.get("github_token") else None,
        }
    else:
    ...
    # Trigger pre-analyze hook
    await plugin_manager.execute_hook("pre_analyze_pr", pr_number, repo, sanitized_title)
    ...
    # Trigger Integrations (Phase 3)
    try:
        await ChatOpsService.notify_pr_analysis(
            pr_number=pr_number, 
            repo=repo, 
            title=summary.title, 
            quality_score=summary.quality_score, 
            workflow_severity=summary.workflow_impact.severity.value
        )
        
        if summary.workflow_impact.severity.value == "major" or summary.quality_score < 70:
            await IssueTrackerService.create_violation_ticket(
                pr_number=pr_number,
                repo=repo,
                title=summary.title,
                changes_summary=summary.summary
            )
    except Exception as e:
        logger.error(f"Error triggering integrations for PR {pr_number}: {e}")

    # Trigger post-analyze hook
    await plugin_manager.execute_hook("post_analyze_pr", pr_number, repo, summary.quality_score)
...
async def _core_changelog(repo: str, pr_number: int) -> dict:
    ...
    import json
    from extension.backend.services.github_service import GitHubService
    from extension.backend.services.changelog_service import ChangelogService
    ...
    try:
        pr_details = await github_service.get_pr_details(repo, pr_number)
        from extension.backend.database import execute
        await execute(
