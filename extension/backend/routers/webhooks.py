...
from extension.backend.middleware.security import verify_github_signature, verify_api_key
from extension.backend.models.schemas import JenkinsWebhookPayload
from extension.backend.config import settings
from extension.backend.worker import celery_app
from extension.backend.tasks import analyze_pr_task, generate_changelog_task
...
from extension.backend.services.pr_analysis import run_pr_analysis
    ...
    try:
        ...
        if _use_sync_processing():
            ...
            if action in ["opened", "reopened", "synchronize"]:
                ...
                gh_svc = GitHubService(token=decrypt_token(row["github_token"])) if row and row.get("github_token") else github_service
            ...
            if action == "closed":
                ...
                if merged:
                    ...
                    if p_row and p_row.get("github_token"):
                        gh_svc = GitHubService(token=decrypt_token(p_row["github_token"]))
