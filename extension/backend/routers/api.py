...
from extension.backend.middleware.security import verify_api_key, validate_repo_name
...
from extension.backend.models.schemas import PRSummary, WorkflowImpact, ChangelogEntry, ChangeItem, Severity, ChangeType, RepoSetupRequest
...
@router.get("/pr/{pr_number}/diff", dependencies=[Depends(verify_api_key)])
async def get_pr_raw_diff(pr_number: int, repo: str = Depends(validate_repo)):
    """
    Fetches the raw unified diff for a PR from GitHub.
    """
    try:
        diff_text = await github_service.get_pr_diff(repo, pr_number)
...
async def generate_and_push_changelog(pr_number: int, repo: Optional[str] = None):
    ...
    if not row:
        try:
            from extension.backend.routers.webhooks import run_pr_analysis
...
async def approve_pr(pr_number: int, repo: str = Depends(validate_repo)):
    ...
    """
    from extension.backend.database import execute_query
...
async def generate_workflow_diagram(payload: dict):
    ...
    try:
        ...
        image_url = f"https://quickchart.io/mermaid?graph={encoded}"
