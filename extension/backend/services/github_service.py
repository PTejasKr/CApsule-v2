...
class GitHubService:
    ...
    async def create_status_check(self, repo: str, sha: str, state: str, description: str, context: str = "Capsule AI Review", target_url: Optional[str] = None) -> Dict[str, Any]:
        """Creates or updates a commit status check on GitHub."""
        logger.info(f"Setting status check '{state}' for {repo} at {sha}")
    ...
    async def post_pr_comment(self, repo: str, pr_number: int, body: str) -> Dict[str, Any]:
        ...
        logger.info(f"Posting comment to PR #{pr_number} on {repo}")
