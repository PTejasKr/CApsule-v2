...
from extension.backend.models.schemas import PRSummary, ChangeItem, WorkflowImpact, ChangeType, Severity
from extension.backend.database import insert
from extension.backend.services.routing_service import router_service
...
class AIEngine:
    ...
    def _build_system_prompt(self, brd_content: str, custom_rules: Optional[str] = None) -> str:
        """
        Builds the grounded system prompt with BRD context, anti-hallucination guidelines, and custom rules.
        """
...
6. Provide a 'reasoning_trace' outlining step-by-step logic for your change evaluation and workflow impact.
{chr(10) + "=== CUSTOM RULES ===" + chr(10) + custom_rules + chr(10) + "=== END OF CUSTOM RULES ===" + chr(10) if custom_rules else ""}
...
{{
  ...
  "changes": [
    {{
      ...
      "confidence": 0.95,
      "reasoning_trace": "Detailed reasoning for this change"
    }}
  ],
  "workflow_impact": {{
    ...
    "reasoning_trace": "Detailed reasoning for this impact"
  }},
  "confidence_score": 0.98,
  "quality_score": 100
}}
The quality_score should start at 100 and decrease based on the number and severity of issues found (e.g. major workflow impact deducts more points).
"""
    ...
    async def _analyze_chunk(self, chunk: str, brd_content: str, model: str = None, custom_rules: Optional[str] = None) -> Dict[str, Any]:
        """Analyze a single diff chunk and return the raw JSON output from the LLM."""
        system_prompt = self._build_system_prompt(brd_content, custom_rules)
    ...
    async def _reduce_overall(self, combined: Dict[str, Any], brd_content: str, model: str = None, custom_rules: Optional[str] = None) -> Dict[str, Any]:
        ...
        system_prompt = (
            ...
            '  "changes": [ { "file": "path/to/file.py", "line_range": "12-25", "change_type": "added|modified|deleted", "description": "...", "confidence": 0.95, "reasoning_trace": "..." } ],\n'
            '  "workflow_impact": { "has_impact": true|false, "severity": "none|minor|major", "impact_description": "...", "affected_workflows": ["..."], "before_state": "...", "after_state": "...", "reasoning_trace": "..." },\n'
            '  "confidence_score": 0.98,\n'
            '  "quality_score": 100\n'
            "}\n"
            "The quality_score should start at 100 and decrease based on the number and severity of issues found (e.g. major workflow impact deducts more points)."
    ...
    async def analyze_pr(self, pr_number: int, repo: str, pr_title: str, diff: str, brd_content: str, branch_name: Optional[str] = None, model: str = None, custom_rules: Optional[str] = None) -> PRSummary:
        ...
        tasks = [self._analyze_chunk(chunk, brd_content, target_model, custom_rules) for chunk in chunks]
        ...
        if getattr(settings, "GLOBAL_REDUCE_ENABLED", True):
            try:
                combined = await self._reduce_overall(combined, brd_content, target_model, custom_rules)
            except Exception as e:
        ...
        workflow_impact = WorkflowImpact(
            ...
            reasoning_trace=wf_data.get("reasoning_trace")
        )
        return PRSummary(
            pr_number=pr_number,
            repo=repo,
            ...
            quality_score=int(validated_data.get("quality_score", 100)),
