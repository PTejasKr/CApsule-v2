...
from extension.backend.services.crypto import encrypt_token
...
async def github_login(request: Request, profile_id: int = 1):
    ...
    if not client_id:
        logger.warning("GITHUB_CLIENT_ID not configured, using mock development login.")
        jwt_payload = {
            "github_username": "dev_user",
            "global_role": "super_admin",
            "exp": time.time() + 3600 * 24
        }
        secret = getattr(settings, "JWT_SECRET_KEY", "fallback_secret")
...
async def github_callback(request: Request, code: str, state: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if user_res.status_code == 200:
            github_username = user_res.json().get("login", "unknown")
            
    try:
        encrypted_token = encrypt_token(access_token)
        await execute("UPDATE profiles SET github_token = ? WHERE id = ?", (encrypted_token, profile_id))
