...
@patch("extension.backend.database.execute_query", new_callable=AsyncMock)
...
@patch("extension.backend.routers.api.fetch_one", new_callable=AsyncMock)
@patch("extension.backend.services.ai_engine.AIEngine.compare_summaries", new_callable=AsyncMock)
...
@patch("extension.backend.database.execute_query", new_callable=AsyncMock)
@patch("extension.backend.routers.api.generate_and_push_changelog", new_callable=AsyncMock)
...
async def test_failover_routing():
    from extension.backend.services.routing_service import MultiProviderRouter
